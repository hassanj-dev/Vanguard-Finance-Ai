  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span className="rounded-full bg-white" style={{ width: size * 0.12, height: size * 0.24 }} />
          <span className="rounded-full bg-white" style={{ width: size * 0.12, height: size * 0.24 }} />
        </span>
...
// Hoisted out of the component — this string was being re-created on every
// single render (including every streamed token).
const BUDDY_CSS = `
...
export default function AIBuddy() {
  ...
  // The SpeechRecognition instance is kept in a ref so it can actually be
  // stopped — otherwise closing the chat mid-listen left the mic running.
  const recognitionRef = useRef<any>(null);

  /**
   * FIX #2 (client side) — the old version destructured only
   * `{ messages, sendMessage, status }`, so when the request failed there
   * was nothing to render and nothing to retry with: the window just sat
   * on "Thinking…" forever with no indication anything had gone wrong.
   *
   * We now take `error` and `regenerate` as well, and show both.
   */
  const { messages, sendMessage, status, error, regenerate } = useChat({
    // Default endpoint is /api/chat. If you ever move the route, pass a
    // transport here instead:
    //   transport: new DefaultChatTransport({ api: '/api/your-route' })
    onError: (err) => {
      console.error('[AIBuddy] chat error:', err);
    },
  });
  ...
  // Auto-scroll to the newest message. Without this, a reply longer than the
  // visible area looked like "no reply" until the user scrolled manually.
  useEffect(() => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 4 * 1024 * 1024) {
      setLocalError('That image is over 4 MB. Try a smaller one.');
      return;
    }
  ...
  const toggleVoiceRecognition = () => {
    ...
    if (!SpeechRecognition) {
      // Was a blocking alert() — it freezes the tab and doesn't match the
      // inline-error pattern used everywhere else in the app.
    ...
    recognition.onerror = () => {
      setIsVoiceActive(false);
      setLocalError("Couldn't hear that. Check your microphone permission.");
    };
    recognition.onend = () => {
      setIsVoiceActive(false);
      recognitionRef.current = null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // useChat surfaces most failures through `error`, but a transport-level
      // throw (offline, DNS) can land here.
      console.error('[AIBuddy] send failed:', err);
      setLocalError('Message could not be sent. Check your connection.');
    }
  };

  // `useChat` streams messages as `parts` (not a flat `content` string).
  // The `m.content` branch is a defensive fallback for older cached shapes.
  ...
  const errorText = localError ?? (error ? error.message || 'Echo could not reply.' : null);
    ...
    <div className="fixed bottom-6 right-6 z-[60]">
      ...
      {!isOpen && (
        <button
          ...
          aria-label="Open Echo, your AI assistant"
      ...
      {isOpen && (
        <div
          className="buddy-window flex w-[420px] max-w-[calc(100vw-3rem)] flex-col rounded-[24px] border border-[var(--line)] bg-[var(--panel)] shadow-[var(--shadow)] overflow-hidden"
        ...
        >
          ...
          <div
            ref={scrollRef}
            className="buddy-scroll flex-1 overflow-y-auto p-4 space-y-4 bg-[var(--panel)] text-xs"
          >
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center my-2 text-center">
                {/* Animated AI Buddy hero */}
            ...
            {messages.map((m) => (
              ...
              >
                ...
                <div
                  className={`rounded-2xl px-4 py-2.5 max-w-[82%] leading-relaxed break-words ${
                    m.role === 'user'
            ...
            {/* Error state — previously there was none at all, which is why
                a failing request looked like "the AI is not replying". */}
            {errorText && !isLoading && (
              <div className="flex items-start gap-2 rounded-2xl border border-[color-mix(in_srgb,var(--over)_30%,transparent)] bg-[color-mix(in_srgb,var(--over)_10%,transparent)] px-3 py-2.5 text-[11px] text-[var(--over)]">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {error && (
                    <button
                      ...
                      className="mt-1.5 inline-flex items-center gap-1 rounded-full border border-[color-mix(in_srgb,var(--over)_35%,transparent)] px-2 py-1 text-[10px] font-semibold transition hover:bg-[color-mix(in_srgb,var(--over)_14%,transparent)] cursor-pointer"
                    >
          ...
          <form onSubmit={handleSubmit} className="p-3">
            <div className="flex items-end gap-1.5 rounded-[20px] border border-[var(--line)] bg-[var(--panel-2)] px-2 py-2 focus-within:border-[var(--line-strong)] transition-colors">
              ...
              <input
                ...
                placeholder={
                  isVoiceActive ? 'Listening...' : selectedFile ? 'Add a note...' : 'Type a message...'
                }
                className="flex-1 min-w-0 bg-transparent px-1 py-1.5 text-xs text-[var(--text)] placeholder:text-[var(--muted)] focus:outline-none"
              />
              ...
              <button
                ...
                className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[var(--accent)] text-white disabled:bg-[var(--panel)] disabled:text-[var(--muted)] disabled:cursor-not-allowed transition cursor-pointer"
              >
