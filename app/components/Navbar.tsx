...
// FIXED: previously hardcoded a random Pinterest image URL as "the" user
// avatar for every account, and pointed at a third-party host with no
// content guarantees. Now derives an initials avatar from the signed-in
// user's own email, same as most real dashboards do without needing a
// dedicated avatar-upload feature.
...
// Uses the real `next_billing_date` column — flags anything renewing within
// this many days.
...
export default function Navbar({ onMenuClick }: NavbarProps) {
  ...
  // Pulls this user's subscriptions and flags anything renewing soon.
  useEffect(() => {
    if (!userId) return;
    ...
    supabase
      ...
      .then(({ data, error }) => {
        ...
        const soon = data
          .map((sub: { id: number; name: string; cost: number; next_billing_date: string }) => {
  ...
  // FIXED: search input previously had no handler at all — it looked
  // interactive but pressing Enter did nothing. It now routes to /search
  // (an existing Sidebar destination) with the query as a URL param, so
  // the /search page can read it via useSearchParams.
  ...
  return (
    <header className="w-full bg-[var(--bg-2)] px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between border-b border-[var(--line)] gap-3">
      ...
      <div className="flex items-center gap-2 sm:gap-3 shrink-0">
        {/* Search — full input on md+, icon-only toggle on mobile */}
        <div className="hidden md:flex relative items-center">
          ...
          <input
            ...
            className="pl-9 pr-4 py-2 bg-[var(--panel)] border border-[var(--line)] rounded-full text-xs text-[var(--text)] placeholder-[var(--muted)] focus:outline-none focus:ring-2 focus:ring-[var(--accent-wash)] focus:border-[var(--line-strong)] w-44 transition-all"
          />
        </div>
        ...
        <div className="relative" ref={notifRef}>
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-2 w-64 bg-[var(--panel)] border border-[var(--line)] rounded-2xl shadow-[var(--shadow)] p-2 z-50">
              <p className="px-2 pb-2 pt-1 text-[11px] font-semibold text-[var(--text)]">Alerts</p>

              {alerts.length === 0 ? (
                <p className="px-2 py-3 text-[11px] text-[var(--muted)]">
                  No alerts right now. We&apos;ll flag subscriptions renewing within {ALERT_WINDOW_DAYS} days.
                </p>
              ) : (
                <ul className="space-y-1">
                  {alerts.map((a) => (
                    ...
                    >
                      <span className="font-semibold capitalize">{a.name}</span> renews in{' '}
                      <span className="font-semibold">
                        {a.daysUntilRenewal === 0 ? 'today' : `${a.daysUntilRenewal}d`}
                      </span>{' '}
                      — ${a.cost}
                    </li>
                  ))}
        ...
        <div className="relative" ref={menuRef}>
          <button
            ...
            className="w-9 h-9 rounded-full overflow-hidden border border-[var(--line)] focus:ring-2 focus:ring-[var(--accent-wash)] flex items-center justify-center bg-[var(--accent-wash)] text-[11px] font-bold text-[var(--accent-soft)]"
          ...
          {showProfileMenu && (
            <div className="absolute right-0 mt-2 w-48 bg-[var(--panel)] border border-[var(--line)] rounded-2xl shadow-[var(--shadow)] p-2 z-50">
              {userEmail && (
      ...
      {showMobileSearch && (
        <div className="md:hidden absolute left-0 right-0 top-full bg-[var(--bg-2)] border-b border-[var(--line)] px-4 py-3">
          <div className="relative flex items-center">
            ...
            <input
              ...
              onChange={(e) => setSearchQuery(e.target.value)}
              onKeyDown={handleSearchKeyDown}
              className="w-full pl-9 pr-4 py-2 bg-[var(--panel)] border border-[var(--line)] rounded-full text-xs text-[var(--text)] placeholder-[var(--muted)] focus:outline-none focus:ring-2 focus:ring-[var(--accent-wash)]"
            />
          </div>
        </div>
      )}
